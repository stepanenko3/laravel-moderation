...
```php
composer require stepanenko3/laravel-moderation
```

If you are using Laravel < 5.5, you need to add Stepanenko3\Moderation\ModerationServiceProvider to your `config/app.php` providers array:
```php
'providers' => [
    ...
    Stepanenko3\Moderation\ModerationServiceProvider::class,
    ...
];
```
...
```
php artisan vendor:publish --provider="Stepanenko3\Moderation\ModerationServiceProvider" --tag=config
```


## Prepare Model

To enable moderation for a model, use the `Stepanenko3\Moderation\Moderatable` trait on the model and add the `status`, `moderated_by` and `moderated_at` columns to your model's table.
```php
use Stepanenko3\Moderation\Moderatable;
